...
from utils.loss_utils import l1_loss, ssim, ScaleAndShiftLoss, l1_loss_masked, ssim_masked, l2_loss, my_ssim, binary_cross_entropy_loss, perceptual_loss, gradient_domain_consistency_loss
...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if gt_image_name == ref_name:
            ...
            # Add perceptual loss for reference view
            if opt.perceptual_loss_weight > 0:
                loss += opt.perceptual_loss_weight * perceptual_loss(image, gt_image, gt_mask)
            
            # Add gradient domain consistency loss for reference view
            if opt.gradient_domain_loss_weight > 0:
                loss += opt.gradient_domain_loss_weight * gradient_domain_consistency_loss(image, gt_image, gt_mask)
        else:
            ...
            # Add perceptual loss for non-reference views (only on visible regions)
            if opt.perceptual_loss_weight > 0:
                visible_mask = 1.0 - gt_mask  # Only apply to background regions
                loss += 0.5 * opt.perceptual_loss_weight * perceptual_loss(image, gt_image, visible_mask)
            
            # Add gradient domain consistency loss for non-reference views (only on visible regions)
            if opt.gradient_domain_loss_weight > 0:
                visible_mask = 1.0 - gt_mask  # Only apply to background regions
                loss += 0.5 * opt.gradient_domain_loss_weight * gradient_domain_consistency_loss(image, gt_image, visible_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[30_000])    #initial 30_000
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[30_000])    #initial 30_000
